...
async function main () {
  ...
  let ethAmount = new BN(10).pow(new BN(18));
  ...
  if (tokenWithFeeAddress == undefined) {
    tokenWithFee = await MockFeeOnTransferERC20.new('Test Fee Token', 'FTST', new BN(10).pow(new BN(32)), { gasPrice: gasPrice });
  ...
  await uniswapRouter.addLiquidityETH(testTokenAddress, tokenAmount, 0, 0, deployer, deadline, { value: ethAmount, gasPrice: gasPrice });
  console.log(`Added liquidity ETH - token`);

  await uniswapRouter.addLiquidity(testTokenAddress, tokenWithFeeAddress, tokenAmount, tokenAmount.div(new BN(2)), 0, 0, deployer, deadline, { gasPrice: gasPrice });
  ...
  await migrator.migrateLpToDmmPool(
    tokenLpPairAddress,
    wethAddress, testToken.address, balance.div(new BN(2)), 0, 0, ["0x0000000000000000000000000000000000000000", 12345], deadline,
    { gasPrice: gasPrice, gas: 6000000 }
  );
  console.log(`Migrated to new Kyber DMM pool`);

  // make a swap to change the rate in Uniswap
  await uniswapRouter.swapExactETHForTokens(0, [wethAddress, testToken.address], deployer, deadline, { value: ethAmount.div(new BN(10)), gasPrice: gasPrice, gas: 200000 });
  let pools = await dmmFactory.getPools(wethAddress, testToken.address);
  console.log(`Kyber DMM pool for token: ${pools[0]}`)

  await migrator.migrateLpToDmmPool(
    tokenLpPairAddress,
    wethAddress, testToken.address, balance.div(new BN(2)), 0, 0, [pools[0], 0], deadline,
    { gasPrice: gasPrice, gas: 6000000 }
  );
  ...
  await migrator.migrateLpToDmmPool(
    tokenLpPairAddress,
    tokenWithFee.address, testToken.address, balance.div(new BN(2)), 0, 0, ["0x0000000000000000000000000000000000000000", 23456], deadline,
    { gasPrice: gasPrice, gas: 6000000 }
  );
  console.log(`Migrated to new Kyber DMM pool`);
  ...
  await uniswapRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(
    new BN(1000000), 0, [tokenWithFee.address, testToken.address], deployer, deadline, { gasPrice: gasPrice, gas: 500000 }
  );
  ...
  await migrator.migrateLpToDmmPool(
    tokenLpPairAddress,
    tokenWithFee.address, testToken.address, balance.div(new BN(2)), 0, 0, [pools[0], 0], deadline,
    { gasPrice: gasPrice, gas: 6000000 }
  );
