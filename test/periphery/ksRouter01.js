...
contract('KSRouter01', function (accounts) {
  ...
  beforeEach('setup', async () => {
    ...
    await factory.createPool(token0.address, token1.address, Helper.KS_BPS, new BN(10));
    ...
    await factory.createPool(weth.address, ethPartner.address, Helper.KS_BPS, new BN(10));
  ...
  describe('test query rate function', async () => {
    it('getAmountOut', async () => {
      let [factory, pool] = await setupPool(feeToSetter, token0, token1, new BN(200000), new BN(10));
    ...
    it('getAmountIn', async () => {
      [factory, pool] = await setupPool(feeToSetter, token0, token1, new BN(200000), new BN(10));
      let router = await KSRouter.new(factory.address, weth.address);
      const token0Amount = Helper.expandTo18Decimals(5);
      const token1Amount = Helper.expandTo18Decimals(10);
      const swapAmount = Helper.expandTo18Decimals(1);
  ...
  describe('special case: token balance > real reserve in the pool', async () => {
    it('addLiquidity', async () => {
      ...
      // create a new pool
      await factory.createPool(token0.address, token1.address, new BN(200000), new BN(10));
      const poolAddrs = await factory.getPools(token0.address, token1.address);
